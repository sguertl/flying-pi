 ...
 * V0.1  : 10-07-2015, ES:  Added Bluetooth Support
 * V0.2  : 21-07-2016, DW:  Port of SW from DAVE3 to DAVE4
 * V0.3  : 14-11-2016, RT:  Moved #defines to Configuration File
 */
...
#include "../Sensors/Radar/Flightcontroller_Radar_Interface.h"
#include "../AltitudeHold/AltitudeHold.h"
...
/**
 * @brief Structure for receiving the raspberry values
 */
typedef struct QUADROCOPTER_RC_RECEIVE_RASPBERRY_VALUES_
{
	uint16_t throttle;		/**< throttle value */
	uint8_t heightControl;	/**< value for height control */
	float rudder;			/**< rudder value */
	float aileron;			/**< aileron value */
	float elevator;
}QUADROCOPTER_RC_RECEIVE_RASPBERRY_VALUES;

typedef struct QUADROCOPTER_RC_TRANSMIT_RASPBERRY_VALUES_
{
	uint16_t throttle;
	uint8_t heightControl;
	float rudder;
	float aileron;
	float elevator;
	...
	float debug3;
	float debug4;
}QUADROCOPTER_RC_TRANSMIT_RASPBERRY_VALUES;
...
*******************************************************************************/
void GetRCData(float* power,uint8_t* altHold_active, float* yaw, float* pitch, float* roll);
