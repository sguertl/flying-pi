...
ControlValue rpi_control_value; /**< struct full of pointer for Raspberry receiving */
uint8_t* ReadBufBT;			/**< Pointer to Bluetooth read Buffer location */
uint8_t* ReadBufRPI;        /**< Pointer to Raspberry read Buffer location */
uint8_t* WriteBufRPI;       /**< Pointer to Raspberry write Buffer location */
/* *****************************************************************************
...
void initBluetoothStorage()
{
	...
	control_value.altHold_active = malloc(sizeof(uint8_t));
	...
	ReadBufRPI = malloc(sizeof(uint8_t)*BLUETOOTH_INPUT_BUFFER);
	WriteBufRPI = malloc(sizeof(uint8_t) * 41);
	rpi_control_value.header = malloc(sizeof(uint8_t));
	rpi_control_value.altHold_active = malloc(sizeof(uint8_t));
	rpi_control_value.speed = malloc(sizeof(uint8_t));
	rpi_control_value.x_pitch = malloc(sizeof(float));
	rpi_control_value.y_roll = malloc(sizeof(float));
	rpi_control_value.z_rotate = malloc(sizeof(float));
	rpi_control_value.checksum = malloc(sizeof(uint32_t));
}
		...
		ControlValue *control_value, DataPacket* packet)
{
	...
	if (*(control_value->checksum) == checksum)//received checksum is the same as calculated checksum
	{
		...
		{
			*((uint8_t*)(control_value->altHold_active)) = input_buffer[1];
